...
resource "aws_lambda_function" "report_job" {
  ...
  code_sha256   = data.archive_file.report_job.output_base64sha256
  runtime       = var.lambda_report_job.runtime
  layers        = [aws_lambda_layer_version.node_modules.arn]

  vpc_config {
    subnet_ids         = data.aws_subnets.private_subnets.ids
    security_group_ids = [aws_security_group.rds.id, aws_security_group.documentdb.id]
  }
  ...
  environment {
    variables = {
      ...
      DOCUMENTDB_DATABASE_NAME   = var.documentdb_cluster.database_name
